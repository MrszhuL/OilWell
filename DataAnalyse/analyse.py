...
def plott(temper,name,SetInterval=[29,31]):#################change
    ...
    plt.annotate('['+str(minn)+', '+str(round(temper[minn],4))+']',xy=(minn,temper[minn]),xytext=(minn,temper[minn]))
    maxx=np.argmax(temper)
    plt.annotate('['+str(maxx)+', '+str(round(temper[maxx],4))+']',xy=(maxx,temper[maxx]),xytext=(maxx,temper[maxx]))
